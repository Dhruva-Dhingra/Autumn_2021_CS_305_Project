...
// 
#ifdef POPT
extern uint32_t curr_vertex, num_vertex;
// extern uint64_t irreg_base, irreg_bound;
extern uint32_t** reref_matrix;
#endif
...
public:
  ...
  uint8_t cache_type;
  uint64_t irreg_base, irreg_bound;
  // prefetch stats
  ...
  uint32_t get_set(uint64_t address), get_way(uint64_t address, uint32_t set),
      ...
      void registerGraph(bool isPull);
      ...
      void insertData(int setVal, int wayVal, int clineID, int epochNum, int EPOCH_NUM);
...
// per-datatype LLC hit/miss characterization
    ...
    Graph m_graph;
    bool m_isPull;
    int32_t m_regIndex[numThreads]; // TODO:need to intialise to -1
    //m_regIndex[0] = -1;
    // PIN_MUTEX m_dstLock;
    ...
    int32_t SET_BASE = 0;
    int32_t WAY_BASE = 0;
    int num_reserved_ways = 0;
    int32_t locationCurr = 0;
