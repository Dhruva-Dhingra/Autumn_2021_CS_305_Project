...
public:
  ...
  // POPT
  uint8_t rerefCol[64];
      //@Rahul: Replacement policy with trip count
...
public:
  ...
  fastset l1_rq_index_depend_on_me, l1_wq_index_depend_on_me,
      l1_pq_index_depend_on_me;
  uint8_t read_translation_merged, write_translation_merged,
      prefetch_translation_merged;
  int l1_rq_index, l1_wq_index, l1_pq_index;
  uint64_t full_physical_address;
  bool send_both_tlb;   // For STLB (if true, STLB should return translation to
                        // both DTLB and ITLB)
  bool send_both_cache; // For L2C (if true, L2C should return data to both L1D
                        // and L1C)

  //@Vishal: PTW
  uint64_t full_virtual_address;
  uint8_t translation_level, init_translation_level;

  uint32_t cpu, data_index, lq_index, sq_index;

  uint64_t address, full_addr, instruction_pa, data_pa, data, instr_id,
      prefetch_id, //@v
      ip, event_cycle, cycle_enqueued;
