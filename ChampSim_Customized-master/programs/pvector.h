...
Only modification - uses posix_memalign for memory allocation
*/
...
class pvector {
 public:
  ...
  explicit pvector(size_t num_elements) {
    ...
    int ret = posix_memalign((void**) &start_, 64, num_elements * sizeof(T_));
    assert(ret == 0 && "Memory allocation failure\n");
  ...
  void reserve(size_t num_elements) {
    if (num_elements > capacity()) {
      ...
      T_ *new_range {nullptr};
      int ret = posix_memalign((void**) &new_range, 64, num_elements * sizeof(T_));
      assert(ret == 0 && "Memory allocation failure\n");
      //#pragma omp parallel for
