  ...
  while (continue_reading) {
    ...
    if (knob_cloudsuite) {
      ...
      if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
        ...
        #ifdef POPT
        if(current_cloudsuite_instr.is_special_instruction == 0){
        #endif
        ...
        else if(current_cloudsuite_instr.is_special_instruction == 1){
          uncore.LLC.registerDataType(current_cloudsuite_instr.addr, current_cloudsuite_instr.dType, current_cloudsuite_instr.numElements, current_cloudsuite_instr.elemSz, current_cloudsuite_instr.totalDataTypes);
        }
        else if(current_cloudsuite_instr.is_special_instruction == 2){
          uncore.LLC.registerGraph(current_cloudsuite_instr.isPull);
        }
        else if(current_cloudsuite_instr.is_special_instruction == 3){
          uncore.LLC.registerOffsetMatrix( current_cloudsuite_instr.numEpochs, current_cloudsuite_instr.numCacheLines, current_cloudsuite_instr.vtxPerLine, current_cloudsuite_instr.dTypeID);
        }
        else if(current_cloudsuite_instr.is_special_instruction == 4){
          uncore.LLC.updateRegIndex(current_cloudsuite_instr.index, current_cloudsuite_instr.tid);
        }
        else if(current_cloudsuite_instr.is_special_instruction == 5){
          uncore.LLC.updateIrregRanges(current_cloudsuite_instr.startID, current_cloudsuite_instr.endID);
        }
        #endif
      }
    } else {
      ...
      if (!fread(&trace_read_instr, instr_size, 1, trace_file)) {
        ...
        // std::cout << "special isntruction = " << current_instr.is_special_instruction << std::endl;
        // std::cout << "ip = " << current_instr.ip << std::endl;
        // std::cout << "is_branch = " << int(current_instr.is_branch) << std::endl;
        // std::cout << "branch_taken = " << int(current_instr.branch_taken) << std::endl;
        if(current_instr.is_special_instruction == 0)
        {
        #endif
        // copy the instruction into the performance model's instruction format
        ooo_model_instr arch_instr;
        ...
        else if(current_instr.is_special_instruction == 1){
          uncore.LLC.registerDataType(current_instr.addr, current_instr.dType, current_instr.numElements, current_instr.elemSz, current_instr.totalDataTypes);
        }
        else if(current_instr.is_special_instruction == 2){
          uncore.LLC.registerGraph(current_instr.isPull);
        }
        else if(current_instr.is_special_instruction == 3){
          uncore.LLC.registerOffsetMatrix( current_instr.numEpochs, current_instr.numCacheLines, current_instr.vtxPerLine, current_instr.dTypeID);
        }
        else if(current_instr.is_special_instruction == 4){
          uncore.LLC.updateRegIndex(current_instr.index, current_instr.tid);
        }
        else if(current_instr.is_special_instruction == 5){
          uncore.LLC.updateIrregRanges(current_instr.startID, current_instr.endID);
        }
        #endif
      }
    }
