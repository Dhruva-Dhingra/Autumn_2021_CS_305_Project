...
std::string graph_file_name = "pr_graph.el";
...
void CACHE::registerDataType(intptr_t addr, int dataTypeID, int numElements, size_t elemSz, int totalDataTypes)
{
    // std::cout << "Entering registerDataType" << std::endl;
    ...
    // std::cout << "Initialised Data Structures" << std::endl;
    ...
    if (dataTypeID == IRREGDATA){
      // std::cout << "3" << std::endl;
      irreg_base = addr;
      irreg_bound = elemSz*numElements;
    }
    ...
    // std::cout << "Exited registerDataTypes" << std::endl;
}

void CACHE::registerGraph(bool isPull)
{
    //m_graph.setGraphProperties(g.num_nodes(), g.num_edges(), g.directed());
    //m_graph.setGraphDatastructures(g.out_index(), g.out_neighbors(), g.in_index(), g.in_neighbors());
    m_isPull = isPull;
    int argc = 3;
    char *argv[3];
    ...
    int l = graph_file_name.length();
    argv[2] = (char*)malloc((l + 1) * sizeof(char));
    for(int i = 0; i < l; i++){
      argv[2][i] = graph_file_name[i];
    }
    argv[2][l] = 0;
...
void CACHE::registerOffsetMatrix( int32_t numEpochs, int32_t numCacheLines, int vtxPerLine, int dTypeID)
{
  // std::cout<<"Entered registerOffsetMatrix"<<std::endl;
  ...
  // std::cout<<"1 - registerOffsetMatrix"<<std::endl;
  ...
  b.makeOffsetMatrix(m_graph, offsetMatrix, vtxPerLine, numEpochs, true); 

  for (int i = 0; i < numEpochs * numCacheLines; ++i){
    
    // std::cout<<int(offsetMatrix.data()[i])<<std::endl;
    m_offsetMatrix[dTypeID][i] = int(offsetMatrix.data()[i]);
  }
  ...
  // std::cout<<"Exited registerOffsetMatrix"<<std::endl;
}

void CACHE::insertData(int setVal, int wayVal, int clineID, int epochNum, int EPOCH_NUM){
    // std::cout<<"Entered insertData"<<std::endl;
    for (int i=0;i<64;i++){
      // std::cout<<"insData.."<<std::endl;
      // std::cout<<m_offsetMatrix[IRREGDATA][epochNum*EPOCH_NUM+clineID]<<std::endl;
      block[setVal][wayVal].rerefCol[i] = m_offsetMatrix[IRREGDATA][epochNum*EPOCH_NUM+clineID];
    }

    // std::cout<<"Exited insertData"<<std::endl;
}


void CACHE::updateRegIndex(int32_t index, int tid)
{
    // lastVertex = -1;
    // int locationCurr  = 0;
    currVertex = index; ///??
    if (lastVertex  == -1)
    { 
            int setVal = SET_BASE % NUM_SET;
            int wayVal = WAY_BASE / NUM_SET;
           
            for (int i=0;i<m_numCacheLines[IRREGDATA]; i++ )
            {   setVal %= NUM_SET;
            //--------------
                reserved_ways[wayVal] = true;
                //Add Code to fetch data from rerefmatrix
                // to (setVal, wayVal) from rerefMatrix[i][0]
                insertData(setVal, wayVal, i, 0, m_numEpochs[IRREGDATA]);
            //------------------
                setVal +=1;
                if(setVal == NUM_SET)
                {wayVal +=1; }
            ...
            //--------------
                reserved_ways[wayVal] = true;
                // Code to fetch data from rerefmatrix
                // to (setVal, wayVal) from rerefMatrix[i][1]
                insertData(setVal, wayVal, i, 1, m_numEpochs[IRREGDATA]);
            //------------------
                setVal +=1;
                if(setVal == NUM_SET)
                {wayVal +=1; }
    ...
    else if  (lastVertex != currVertex)
    {
        if ((currVertex%m_numEpochs[IRREGDATA]) ==0 )
        {   
            int currEpoch = currVertex/m_numEpochs[IRREGDATA];
            locationCurr = 1-locationCurr;
            int setVal = (SET_BASE + (1-locationCurr)*m_numCacheLines[IRREGDATA] ) % NUM_SET;
            int wayVal = WAY_BASE + ((1-locationCurr)*m_numCacheLines[IRREGDATA]) / NUM_SET;
            for (int i=0;i<m_numCacheLines[IRREGDATA]; i++ )
            {   setVal %= NUM_SET;
            //--------------
                reserved_ways[wayVal] = true;
                // Code to fetch data from rerefmatrix
                // to (setVal, wayVal) from rerefMatrix[i][currEpoch]
                insertData(setVal, wayVal, i, currEpoch+1, m_numEpochs[IRREGDATA]);
            //------------------
                setVal +=1;
                if(setVal == NUM_SET)
                {wayVal +=1; }
...
void CACHE::updateIrregRanges(int32_t startID, int32_t endID)
{
